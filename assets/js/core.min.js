...
var HaLim = {
  ...
  GetPostByWidgetType: function (
    ...
    $.ajax({
      ...
      error: function (e) {
        $("#ajax-" + widget_type + "-widget-" + widget_id).html(
          "Apparently, there are no posts to show!"
        );
        $(
          ".halim-" +
            widget_type +
  ...
  Actions: function () {
    ...
    $('.episode-pagination a[data-toggle="tab"]').on(
      "shown.bs.tab",
      function (e) {
    ...
    $(".halim_ajax_get_post").on("click", function () {
      ...
      $('<div class="halim-ajax-get-post-loading hidden"></div>').appendTo(
        "#" + widget_id + "-ajax-box"
      );
      $.ajax({
        url: halim.ajax_url,
        ...
        beforeSend: function () {
          $(
            "#" + widget_id + "-ajax-box .halim-ajax-get-post-loading"
          ).removeClass("hidden");
        },
        success: function (data) {
    ...
    $(".halim_ajax_get_schedule").on("click", function () {
      var widget_id = $(this).data("widgetid");
      var tab_Id = $(this).data("day");
      $(".halim_ajax_get_schedule").removeClass("active");
      $(".tab-schedule").removeClass("active");
    ...
    $(".ajax-tab").click(function () {
      $.ajax({
        ...
        beforeSend: function () {
          $(
            ".halim_tab_popular_videos-widget .halim-ajax-popular-post-loading"
          ).removeClass("hidden");
        },
        success: function (data) {
    ...
    $(".ajax-vertical-widget").click(function () {
      ...
      $.ajax({
        ...
        beforeSend: function () {
          $("." + type + " .halim-ajax-popular-post-loading").removeClass(
            "hidden"
          );
        },
        success: function (data) {
          $("#ajax-vertical-widget-" + type).html(data);
          $("." + type + " .halim-ajax-popular-post-loading").addClass(
            "hidden"
          );
        },
        error: function (e) {
          $("#ajax-vertical-widget-" + type).html(
            "Apparently, there are no posts to show!"
          );
          $("." + type + " .halim-ajax-popular-post-loading").addClass(
            "hidden"
    ...
    $(".ajax-category-widget").click(function () {
      ...
      $.ajax({
        ...
        error: function (e) {
          $("#ajax-category-widget-" + widget_id).html(
            "Apparently, there are no posts to show!"
          );
          $(
            ".halim-category-widget-" +
    ...
    if (window.matchMedia("(max-width: 767px)").matches || is_Mobile()) {
      ...
      $(
        '<div id="bookmark-list" class="hidden"><ul style="margin: 0;"></ul></div>'
      ).insertAfter("#user-info");
      $('<ul class="ui-autocomplete ajax-results hidden"></ul>').insertBefore(
        "#user-info"
      );
    ...
    $(".show-more").click(function () {
      ...
      if ($(this).data("single") == true) {
        ...
        var toggled_icon =
          icon == "hl-angle-down" ? "hl-angle-up" : "hl-angle-down";
    ...
    $("#expand-ajax-filter").click(function () {
      ...
      if ($("#ajax-filter div").hasClass("halim-search-filter") == false) {
        $.ajax({
          ...
          beforeSend: function () {
            $("#ajax-filter-icon")
              .removeClass("hl-angle-down")
              .addClass("animate-spin hl-spin4");
          },
          success: function (data) {
            $("#ajax-filter-icon")
              .removeClass("animate-spin hl-spin4")
              .addClass("hl-angle-down");
  ...
  setHistory: function (item = false) {
    var date = new Date();
    if (typeof DoPostInfo === "undefined") {
      return;
    }
    if (item) {
      DoPostInfo = item;
    }
    var bi_historys = localStorage.getItem("myApp_history");
    bi_historys = bi_historys ? JSON.parse(bi_historys) : [];
    for (var i = 0, j = bi_historys.length; i < j; i++) {
      if (bi_historys[i][0] === DoPostInfo.id) {
        bi_historys.splice(i, 1);
        break;
      }
    }
    bi_historys.unshift([
    ...
    if (bi_historys.length > 10) {
      historys = bi_historys.slice(0, 10);
    }
    localStorage.setItem("myApp_history", JSON.stringify(bi_historys));
  },
  listHistory: function () {
    ...
    var bi_historys = localStorage.getItem("myApp_history");
    if (bi_historys) {
      bi_historys = JSON.parse(bi_historys);
      if (bi_historys.length) {
        var output = ``;
        for (var i = 0, j = bi_historys.length; i < j; i++) {
          output += `<article class="col-md-3 col-sm-3 col-xs-6 thumb grid-item post-${bi_historys[i][0]}">
          <div class="halim-item">
                        <a class="halim-thumb" href="${bi_historys[i][1]}">
                            <figure>
                                <img src="${bi_historys[i][4]}" />
                            </figure>
                            <div class="halim-post-title-box">
              <div class="halim-post-title">
                <h2 class="entry-title">${bi_historys[i][2]}</h2>
                <p class="original_title">Bạn đang xem ${bi_historys[i][3]}</p>
              </div>
                            </div>
                       </a></div></article>`;
        }
   ...
   listBookmark: function () {
    if (jQuery("#bookmark-list").length <= 0) {
      return;
    }
    var data = localStorage.getItem("halim_bookmark");
    var halim_bookmark = data ? JSON.parse(data) : {};
    if (Object.keys(halim_bookmark).length > 0) {
          $.each(halim_bookmark, function (index, value) {
            $("#bookmark-list").append(
              `<article class="col-md-3 col-sm-3 col-xs-6 thumb grid-item post-${index}">
          <div class="halim-item">
                        <a class="halim-thumb" href="${value["link"]}">
                            <figure>
                                <img src="${value["thumb"]}" />
                            </figure>
                            <div class="halim-post-title-box">
              <div class="halim-post-title">
                ...
                <p class="original_title">Bạn đang xem </p>
              </div>
                            </div>
                       </a></div></article>`
            );
          ...
          $("#bookmark-list").html(
            '<li style="text-align:center;color:#ccc;">Nothing found!</li>'
          );
  },
  ...
  Bookmark: function () {
    if (typeof Storage !== "undefined") {
      var bookmark_numb = 0,
        halim_bookmark = {};
      if (
        localStorage["halim_bookmark"] !== undefined &&
        localStorage["halim_bookmark"] !== "{}"
      ) {
        var halim_bookmark = JSON.parse(localStorage["halim_bookmark"]);
        bookmark_numb = Object.keys(halim_bookmark).length;
        $("#get-bookmark span.count, .get-bookmark-on-mobile span.count").text(
          bookmark_numb
        );
      }
      $("#get-bookmark, .get-bookmark-on-mobile").click(function () {
        ...
        html +=
          '<h3 class="section-title"><span>Bookmarks</span></h3><span class="remove-all-bookmark box-shadow">x Remove all</span>';
        html += "</div>";
          ...
          $.each(halim_bookmark, function (index, value) {
            $("#list-fav").append(
              '<li class="bookmark-list"><a href="' +
                ...
                value["title"] +
                '</span><span class="bookmark-date">' +
                value["date"] +
                '</span></a><span class="remove-bookmark box-shadow" data-post_id="' +
                index +
                '">x</span></li>'
            );
          });
      ...
      if ($("body.single-post").length && halim_bookmark[halim_cfg.post_id]) {
        $("#bookmark")
          .removeClass("bookmark-img-animation")
          .addClass("bookmarked");
      ...
      $("#bookmark").click(function () {
        if (!$(this).hasClass("bookmarked")) {
          ...
          if (halim_bookmark[post_id] == undefined) {
            ...
            bookmark_numb = Object.keys(halim_bookmark).length;
            $(
              "#get-bookmark span.count, .get-bookmark-on-mobile span.count"
            ).text(bookmark_numb);
          }
        } else {
      ...
      $(document).on("click", ".remove-all-bookmark", function () {
        ...
        $("#list-fav").html(
          '<li style="text-align:center;color:#ccc;">Your bookmark has been deleted!</li>'
        );
        $(".count").text("0");
  ...
  ProfilesBookmark: function () {
    if (typeof Storage !== "undefined") {
        ...
        $.each(halim_bookmark, function (index, value) {
          $("#bookmarkList").append(
            '<li class="bookmark-list"><a href="' +
              ...
              '</span><span class="bookmark-date">' +
              value["date"] +
              '</span></a><span class="remove-bookmark box-shadow" id="' +
              index +
              '">x</span></li>'
          );
        });
      ...
      $(document).on(
        "click",
        "#bookmarkList .bookmark-list .remove-bookmark",
        function () {
          ...
          bookmark_numb = Object.keys(halim_bookmark).length;
          $(
            "#get-bookmark span.count, .get-bookmark-on-mobile span.count i"
          ).text(bookmark_numb);
        }
      );
    }
  ...
  isAdult: function () {
    if (typeof halim_cfg !== "undefined") {
      var html =
        '<div id="is_adultModal" class="modal fade" role="dialog">' +
        '          <div class="modal-dialog modal-lg">' +
        '            <div class="modal-content">' +
        '                <div class="modal-header">' +
        '                    <button type="button" class="close" data-dismiss="modal">×</button>' +
        '                    <h4 class="modal-title adult-content-title">' +
        ...
        '                        <label><input class="modal-check" name="modal-check" type="checkbox"> ' +
        halim_cfg.show_only_once +
        "</label>" +
        "                    </div>" +
        '                    <button type="button" class="btn btn-danger"><a href="/" style="color: white;">' +
        halim_cfg.exit_btn +
        "</a></button>" +
  ...
  LoadEpList: function (post_id, server, cur_sv, eps_nav, episode, eps_link) {
    $.ajax({
      ...
      beforeSend: function () {
        $(".list-episode-ajax-" + server).html(
          '<div class="text-center"><img src="' +
            halim_cfg.loading_img +
            '" /></div>'
        );
      },
      success: function (result) {
  ...
  GetEpsNav: function () {
    if ($("body.single-post").length) {
      if (
        halim_cfg.paging_episode == "true" ||
        halim_cfg.episode_display == "show_paging_eps"
      ) {
  ...
  Rating: function () {
    $(".review-percentage .review-item span").each(function () {
      ($g = $(this).find("span").attr("data-width")),
        HaLim.Progress($g, $(this));
    }),
      $(document).on("mousemove", ".user-rate-active", function (a) {
        ...
        var c = a.offsetX + 4;
        c > 100 && (c = 100),
          b.find(".user-rate-image span").css("width", c + "%");
      ...
      $(document).on("click", ".user-rate-active", function () {
        ...
        a.find(".user-rate-image").hide(),
          a.append('<span class="taq-load"></span>'),
          b > 100 && (b = 100),
          (ngg = (5 * b) / 100);
        ...
        return (
          $.post(
            ...
            function (data) {
              console.log(data);
              a.addClass("rated-done").attr("data-rate", b),
                a.find(".user-rate-image span").width(b + "%"),
